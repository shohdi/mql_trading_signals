...
input int averageMoveMultiply = 2.0;
...
void  getSma(int pos,double &sma[])
{
   ArrayResize(sma,3);
   sma[0] = shohdiSma(pos,InPeriod,longPeriod,0);
   sma[1] = shohdiSma(pos + (longPeriod/2),InPeriod,longPeriod,0);
   sma[2] = shohdiSma(pos + longPeriod,InPeriod,longPeriod,0);
...

{
     ...
     int orderNums = getOpenedOrderNo();
           ...
           
      {
         ...
         double sma2_1 = shohdiSma(longPeriod/10,InPeriod,longPeriod,0);
      ...
      else
      {
         ...
         else if (((foundSignalPrice - vbid) > (takeProfitMove * averageMoveMultiply )))
         {
            
               foundSignalPrice = 0.0;
               foundSignalDir = 0.0;
               for (int i=1;i< checkBack;i++)
               {
                  double sma[];
                  getSma(i,sma);
         ...
         else if (((vbid - foundSignalPrice) > (takeProfitMove * averageMoveMultiply )))
         {
            foundSignalPrice = 0.0;
            foundSignalDir = 0.0;
            for (int j=1;j< checkBack;j++)
            {
               double smaDown[];
               getSma(j,smaDown);
               if(smaDown[0] < smaDown[1] && smaDown[1] < smaDown[2])
                  return -1;
            }
